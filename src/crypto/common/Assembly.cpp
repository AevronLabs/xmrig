...
{
    if (!assembly) return defaultValue;
    for (size_t i = 0; i < MAX; i++) {
        if (strcasecmp(assembly, asmNames[i]) == 0) return static_cast<Id>(i);
    }
    return defaultValue;
}   
...
{
    if (value.IsBool()) return value.GetBool() ? AUTO : NONE;
    if (value.IsString()) return parse(value.GetString(), defaultValue);
    return defaultValue;
}
...
rapidjson::Value xmrig::Assembly::toJSON() const
{
    ...
    if (m_id == NONE) return Value(false);
    if (m_id == AUTO) return Value(true);
